...
import { ArrowDown, ArrowUpRight, ArrowRight, Pause, Play, Mail, Phone } from 'lucide-react';
...
const contactEmail = 'junskam@gmail.com';
const contactPhone = '510 875 101';
const contactPhoneHref = 'tel:+48510875101';
...
export default function Home() {
  ...
  return <div className={motion?'site':'site motion-paused'}>
    ...
    <main id="poczatek">
      ...
      <section className="about-section section-pad marketing-about" id="o-mnie" aria-labelledby="about-title"><figure className="about-photo"><img src="/media/marcelina-junska-portrait.png" width={1080} height={1350} alt="Marcelina Juńska, twórczyni Arcysia Marketing" loading="lazy"/></figure><div className="about-copy"><span className="eyebrow">04 / O MNIE</span><h2 id="about-title">Marcelina<br/><em>Juńska.</em></h2><p>Łączę kreatywność z organizacją. Tworzę komunikację marek — od kierunku wizualnego i strategii po posty, rolki i codzienne social media. Jako freelancerka współpracuję z agencją UX oraz markami z branży beauty i meblarskiej. Dbam o pomysł, plan i szczegóły, które składają się na gotową realizację.</p><div className="about-events"><h3>Za kreatywnością stoi dobry plan.</h3><p>Mam ponad trzyletnie doświadczenie w projektach marketingowych i eventowych. Koordynowałam projekty i konferencje z branży farmaceutycznej w całej Polsce. Organizacja wydarzeń to drugi obszar mojej pracy, który prezentuję w osobnym portfolio.</p><a href="https://marcelina-junska-eventy.arcysia.chatgpt.site" className="about-events-link">Poznaj moje portfolio eventowe <ArrowUpRight size={19}/></a></div></div></section>
      ...
      <section className="contact-section section-pad" id="kontakt" aria-labelledby="contact-title"><div><span className="eyebrow">06 / KONTAKT</span><h2 id="contact-title">Opowiedz mi,<br/>co <em>planujesz.</em></h2><p className="contact-lead">Nowa marka, odświeżenie wizerunku, a może wsparcie w social mediach? Napisz kilka słów o swoim pomyśle.</p><div className="contact-links"><a className="button button-orange" href={`mailto:${contactEmail}`}>Napisz do mnie <Mail size={21} aria-hidden="true"/></a><dl className="contact-details"><div><dt>E-mail</dt><dd><a href={`mailto:${contactEmail}`}><Mail size={20} aria-hidden="true"/>{contactEmail}</a></dd></div><div><dt>Telefon</dt><dd><a href={contactPhoneHref}><Phone size={20} aria-hidden="true"/>{contactPhone}</a></dd></div></dl></div></div><aside className="contact-brief"><span className="eyebrow">DO PRZYGOTOWANIA WYCENY</span><h3>Kilka słów<br/>na dobry początek.</h3><ul><li>Co planujesz i jaki jest Twój cel?</li><li>Jaki zakres wsparcia Cię interesuje?</li><li>Jaki termin i budżet bierzesz pod uwagę?</li><li>Gdzie chcesz publikować swoje materiały?</li></ul></aside></section>
    </main>
