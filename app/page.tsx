  ...
  return <article className="reel-item">
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger className="reel-trigger" aria-label={`Odtwórz: ${item.title}`}>
        <span className="reel-top"><span>{item.category}</span><span>{String(item.id).padStart(2,'0')} / 13</span></span>
...
function PostCard({ item }: { item: typeof posts[number] }) {
  return <article className="post-item reveal"><Dialog>
    <DialogTrigger className="post-trigger" aria-label={`Powiększ grafikę: ${item.title}`}>
      <span className="post-image"><img src={item.src} alt={item.alt} width={1080} height={1440} loading="lazy"/><span className="enlarge"><Maximize2 size={19}/> Powiększ</span></span>
    ...
    <DialogContent className="media-dialog image-dialog" showCloseButton={false}>
      ...
      <img className="full-image" src={item.src} alt={item.alt} width={1080} height={1440}/>
    </DialogContent>
  </Dialog></article>;
}
export default function Home() {
  ...
  return <main className={motion ? '' : 'motion-paused'}>
    ...
    <section className="reels-section" id="rolki" aria-labelledby="reels-title">
      <div className="section-heading reel-heading reveal"><div><span>01 / WIDEO</span><h2 id="reels-title">Zatrzymaj <em>scroll.</em></h2></div><p>Beauty, biznes, wydarzenia i wellness.<br/>Różne marki. Moje spojrzenie.</p></div>
      <div className="gallery-toolbar"><span>13 ROLEK · MOJE REALIZACJE</span><div className="gallery-controls"><span aria-live="polite">{String(activeReel).padStart(2,'0')} <span className="muted">/ 13</span></span><button onClick={() => moveGallery(-1)} aria-label="Poprzednie rolki" disabled={activeReel===1}><ArrowLeft size={21}/></button><button onClick={() => moveGallery(1)} aria-label="Następne rolki" disabled={activeReel===13}><ArrowRight size={21}/></button></div></div>
      <div className="reels-track" ref={track} role="region" aria-label="Galeria 13 rolek. Przewiń w bok, aby zobaczyć kolejne." tabIndex={0} onScroll={() => {
        const el = track.current;if (!el) return;
        const max = el.scrollWidth-el.clientWidth;
        const card=el.querySelector<HTMLElement>('.reel-item');
        setActiveReel(el.scrollLeft>=max-5 ? 13 : Math.min(13,Math.round(el.scrollLeft/((card?.offsetWidth || 300)+24))+1));
      }}>
    ...
    <section id="posty" className="posts-section" aria-labelledby="posts-title"><div className="section-heading posts-heading reveal"><div><span>02 / GRAFIKI</span><h2 id="posts-title">Jeden kadr.<br/><em>Cała historia.</em></h2></div><p>Posty, które dopełniają<br/>wizualny świat marki.</p></div><div className="posts-grid">{posts.map(item=><PostCard key={item.id} item={item}/>)}</div></section>
    <section id="o-mnie" className="about-section"><div className="about-label"><span>03 / ZA KADREM</span><span className="asterisk" aria-hidden="true">✳</span></div><div className="reveal"><p className="about-intro">Cześć, jestem</p><h2>Marcelina<br/><em>Juńska.</em></h2><p className="about-text">Łączę obraz, słowo i ruch. Tworzę posty oraz rolki dla marek — od beauty i wellness po biznes i wydarzenia. Tutaj znajdziesz moje realizacje.</p><span className="about-signature">Arcysia Marketing ↗</span></div></section>
