...
import CommunicationMap from './communication-map';
...
export default function Home() {
  ...
  return <div className={motion?'site':'site motion-paused'}>
    ...
    <header className="site-header"><a className="wordmark" href="#poczatek" aria-label="Arcysia Marketing — początek">arcysia<small>MARKETING</small></a><nav aria-label="Nawigacja główna"><a href="#portfolio">Portfolio</a><a href="#marketing">Oferta</a><a href="#mapa-komunikacji">Mapa komunikacji</a><a href="#o-mnie">O mnie</a><a href="#kontakt">Kontakt</a></nav><button className="motion-toggle" onClick={()=>setMotion(!motion)} aria-pressed={!motion} aria-label={motion?'Wstrzymaj animacje':'Włącz animacje'} title={motion?'Wstrzymaj animacje':'Włącz animacje'}>{motion?<Pause size={17}/>:<Play size={17}/>}</button></header>
    <main id="poczatek">
