  ...
  const content = [
    {
      ...
      topic: `Jak wygląda ${seed.process}?`,
      hook: `Co musi wydarzyć się przed tym, żeby ${seed.focus} zadziałały?`,
      screen: 'Zobacz, co dzieje się przed efektem.',
      cta: `Napisz, jeśli chcesz poznać ${seed.action}.`,
      visual: `Detal procesu: ${seed.scene}.`,
    },
    {
      message: `Wyjaśnij, jak odbiorca może wybrać właściwe rozwiązanie związane z ${seed.focus}.`,
      hint: 'Zbuduj trzy slajdy według kryterium, sytuacji i rekomendacji. Odpowiedz na jedno realne pytanie zakupowe.',
      topic: `Jak wybrać rozwiązanie, kiedy liczy się ${seed.focus}?`,
      hook: 'Nie wiesz, od czego zacząć? Zacznij od tego jednego kryterium.',
      screen: 'Wybór dopasowany do Twojej sytuacji.',
      cta: `Zapisz post i zapytaj o ${seed.action}.`,
      visual: `Porównanie wariantów, detali lub etapów dla ${seed.label.toLowerCase()}.`,
    },
    {
      message: `Opowiedz o konkretnej realizacji, pokazując punkt wyjścia, decyzję i efekt w obszarze ${seed.focus}.`,
      ...
      cta: `Opowiedz o swojej sytuacji i sprawdź ${seed.action}.`,
      visual: `Trzy kadry tej samej realizacji: przed, w trakcie i po.`,
    },
    {
      message: `Odpowiedz na pytanie, które odbiorcy zadają przed skorzystaniem z ${seed.label.toLowerCase()}.`,
      hint: 'Zbierz pytania z wiadomości i rozmów. Jedna karta powinna odpowiadać na jedno pytanie, bez ogólników.',
      topic: `Co warto wiedzieć przed pierwszym kontaktem z ${seed.label.toLowerCase()}?`,
      ...
      visual: `Czytelne pytanie na ekranie i odpowiedź pokazana na tle ${seed.scene}.`,
    },
    {
      message: `Pokaż jedną decyzję, która wyróżnia sposób pracy i wpływa na ${seed.focus}.`,
      hint: 'Wybierz detal, który zwykle pozostaje niewidoczny. Powiedz, co zmienia dla odbiorcy i dlaczego go pilnujesz.',
      topic: `Jedna decyzja, która odróżnia ${seed.label.toLowerCase()}.`,
      hook: 'To mały detal, ale właśnie on zmienia doświadczenie odbiorcy.',
      screen: 'Wyróżnik, który ma znaczenie.',
      cta: `Napisz, jeśli chcesz porozmawiać o ${seed.action}.`,
